...
def format_response_and_fill_field(response, note, target_field):
    ...
    if response is None:
        logger.error("response in null for some reason")
        return
    ...
    if target_field in note:
        ...
        logger.debug("filled the field %s", target_field)
    else:
        logger.error(f"Target field '{target_field}' not found in note.")
...
def fill_field_for_note_in_editor(response, target_field, editor):
    ...
    logger.debug("trying to fill field in editor")
    format_response_and_fill_field(response, editor.note, target_field)
    editor.loadNoteKeepingFocus()
    logger.debug("the editor should be updated")


def fill_field_for_note_not_in_editor(response, note, target_field):
    """Set response to the note."""
    logger.debug("trying to fill field without editor")
    format_response_and_fill_field(response, note, target_field)
    note.flush()
    logger.debug("note is flushed and the changes are supposed to be recorded in the database")
